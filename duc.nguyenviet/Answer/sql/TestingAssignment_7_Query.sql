...
DELIMITER $$
    ...
    BEGIN
		...
		-- IF YEAR(NEW.CreateDate) < YEAR(NOW) - 1
...
-- flow:
-- 1. create function get Id Department when input DepartmentName = Sale;
FUNCTION getDepartmentIdOfSale (
	SELECT  departmentID
	FROM Department
	WHERE DepartmentName = 'Sale'
	) AS departmentID

-- 2. check NEW.DepartmentID = Id --> skip, add message
DROP TRIGGER IF EXISTS trig_Not_Accept_addUserSale;
DELIMITER $$
	CREATE TRIGGER trig_Not_Accept_addUserSale
    BEFORE INSERT ON `Department`
    ...
    BEGIN
		...
		--create function get Id Department when input DepartmentName = Sale;
		DECLARE departmentID = getDepartmentIdOfSale();
		
			IF (NEW.DepartmentID = departmentID) THEN
				-- select,
				-- insert, update, delete
				SIGNAL SQLSTATE '12345'
				SET MESSAGE_TEXT = 'Department "Sale" cannot add more user';
...
BEGIN
		IF ((SELECT Count(1)
			FROM GroupAccount 
			GROUP BY GroupID 
			HAVING Count(AccountID) >= 5) > 1) THEN
			SIGNAL SQLSTATE '12345'
			SET MESSAGE_TEXT = 'Một group có nhiều nhất 5 User';
...
-- hãy tạo trigger cho phép người dùng khi tạo account không điền vào departmentID thì sẽ được phân vào phòng ban "Phòng chờ"
...
-- của account là nam, nữ, chưa xác định thì sẽ đổi lại thành M, F, U cho giống với cấu hình ở database
