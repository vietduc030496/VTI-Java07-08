...
/*QUESTION 17*/
SELECT NV.ACCOUNTID,NV.EMAIL,NV.USERNAME,NV.FULLNAME,NV.DEPARTMENTID,NV.POSITIONID, NV.CREATEDATE, NC.GROUPID
FROM `ACCOUNT` NV
INNER JOIN GROUPACCOUNT NC ON
	(NV.ACCOUNTID = NC.ACCOUNTID)
WHERE NC.GROUPID = 1
UNION 
SELECT NV.ACCOUNTID,NV.EMAIL,NV.USERNAME,NV.FULLNAME,NV.DEPARTMENTID,NV.POSITIONID, NV.CREATEDATE, NC.GROUPID
FROM `ACCOUNT` NV
...
/*QUESTION 18*/
SELECT G.GROUPID,G.GROUPNAME,G.CREATORID, G.CREATEDATE, COUNT(*) AS SO_THANH_VIEN
FROM `GROUP` G
INNER JOIN GROUPACCOUNT NC ON
	(G.GROUPID = NC.GROUPID)
GROUP BY GROUPID
HAVING COUNT(*) > 5
UNION ALL
SELECT G.GROUPID,G.GROUPNAME,G.CREATORID, G.CREATEDATE, COUNT(*) AS SO_THANH_VIEN
